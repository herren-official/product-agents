---
name: b2c-android-issue-stale-checker
description: GitHub 이슈 본문 + 생성일을 받아, 이슈에서 언급한 변경 대상(파일 경로 / 메서드 / 키워드)이 현재 코드/문서 상태와 비교해 이미 해결됐는지 판정. 격리 컨텍스트에서 Read + Grep + `git log -p` 수행 후 stale / valid 판정과 근거만 회수. Use when /from-issue §1.5 사전 stale 체크 필요 시.
...
> 파일 경로가 명시 안 됐어도 모듈명만 언급된 경우 `.docs/module-index.md` 의 33개 모듈 매핑으로 후보 위치 추정 가능.
...
## 실전 사례 (2026-04-22)
이슈 #960 "size → pageSize" 가 PR #926 리뷰 반영 커밋에서 이미 수정된 상태였음. 본 에이전트로 워크플로우 시작 직후 발견 → 이슈 close + 노션 카드 "이슈아님" 처리로 빈 PR 차단.
