...
# b2c-ios-plan 스킬은 별도 오케스트레이션 경로이므로 이 에이전트에서는 직접 호출하지 않음 (역할 분리)
---
...
| 시나리오 | 설명 |
|---------|------|
| orchestrator에서 호출 | Phase 1 분석 결과(b2c-ios-code-analyzer, b2c-ios-design-analyzer 결과)를 prompt로 전달받아 Phase 2~7 수행 |
...
|-------|---------|-------------|
| `b2c-ios-figma-analyze` | Figma 디자인 간단 분석 | Phase 2 (b2c-ios-design-analyzer 결과 없을 시) |
| `b2c-ios-notion-read` | Notion 일감 읽기 및 파싱 | Phase 3 |
| `b2c-ios-feature-explore` | Feature 모듈 구조/패턴 탐색 | Phase 4 |
| `b2c-ios-branch-strategy` | 브랜치 전략 수립 | Phase 6 (Git Strategy) |
| `b2c-ios-notion-update` | Notion 페이지 업데이트 | Phase 7 |
| `b2c-ios-plan` | 작업 계획 오케스트레이션 | Phase 6-7 |
...
> b2c-ios-design-analyzer 에이전트의 분석 결과가 있으면 해당 결과를 활용한다.
> 결과가 없으면 `b2c-ios-figma-analyze` 스킬로 간단 분석만 수행한다 (상세 DesignSystem 매핑은 생략).

- b2c-ios-design-analyzer 결과 있음: Gap Analysis 테이블, 매핑 테이블 직접 활용
- b2c-ios-design-analyzer 결과 없음: b2c-ios-figma-analyze로 화면 구조와 주요 디자인 요소만 파악
...
> b2c-ios-design-analyzer 결과가 있으면 해당 Gap Analysis 테이블을 직접 활용한다.
...
> `b2c-ios-branch-strategy` 스킬의 프로세스를 참조한다
> 참고: `b2c-ios-plan` 스킬은 별도 오케스트레이션 경로이며, 이 에이전트는 독립 실행 또는 orchestrator 경유 실행만 다룬다. `b2c-ios-plan`은 직접 호출하지 않는다.
...
3. **Git Strategy** - `b2c-ios-branch-strategy` 스킬 참조하여 수립
...
Before presenting the work b2c-ios-plan:
...
You have a Persistent Agent Memory directory at `.claude/agent-memory/b2c-ios-task-planner/`. Its contents persist across conversations.
