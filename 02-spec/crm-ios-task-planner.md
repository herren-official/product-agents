...
### 방법 1: 현재 세션에서 실행
대화 중 crm-ios-task-planner 워크플로우를 요청하면 됩니다.
```
"이 자료들로 일감을 생성해줘"
+ 노션/피그마/슬랙 URL 전달
```
...
crm-ios-task-planner를 리드로 지정하여 별도 세션에서 실행합니다.
```bash
claude --agent crm-ios-task-planner
```
...
| teammate | 역할 | 실행 단계 |
|----------|------|----------|
| crm-ios-notion-analyzer | 노션 문서 분석 | Step 2 (병렬) |
| crm-ios-slack-analyzer | 슬랙 링크 분석 | Step 2 (병렬) |
| crm-ios-figma-ui-analyzer | 피그마 UI 분석 | Step 2 (병렬, 섹션별) |
| crm-ios-figma-policy-analyzer | 피그마 정책 분석 | Step 2 (병렬, 섹션별) |
| crm-ios-code-analyzer | 코드 분석 + 개발 범위 | Step 2 (병렬) |
| crm-ios-side-effect-analyzer | 사이드이펙트 분석 | Step 3 (순차) |
...
```
Agent(
  name: "crm-ios-notion-analyzer",
  team_name: "task-planning",
  subagent_type: "crm-ios-notion-analyzer",
  prompt: "..."
)
```
...
- 노션 URL이 있으면 → **crm-ios-notion-analyzer**
- 슬랙 링크가 있으면 → **crm-ios-slack-analyzer**
- 피그마 URL이 있으면 → 섹션별로 **crm-ios-figma-ui-analyzer** + **crm-ios-figma-policy-analyzer** (병렬)
- 키워드가 파악되면 → **crm-ios-code-analyzer** (기존 코드 분석 + 개발 범위 파악)

모든 teammate을 **하나의 메시지에서** 동시에 생성하여 병렬 실행합니다.

### Step 3: 사이드이펙트 분석
Step 2의 결과를 종합하여 **crm-ios-side-effect-analyzer** teammate을 생성합니다.
전달 정보: crm-ios-code-analyzer의 수정/생성 파일 목록, 변경 대상 모델/API/컴포넌트
...
- **영향 범위**: 변경으로 인해 확인해야 할 다른 화면/기능 (crm-ios-side-effect-analyzer 위험도 반영)
- **확인 비용**: 테스트/검증에 드는 시간 (상태 조합, 시나리오 분기 수)

#### Git 계획 규칙
- 에픽 작업: `{에픽명}/GBIZ-{번호}-{작업내용}`
- 일반 작업: `GBIZ-{번호}-{작업내용}`
...
- 속성: 플랫폼 iOS, 서비스 공비서-B2B, 유형 작업, 상태 백로그
