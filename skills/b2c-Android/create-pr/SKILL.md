...
### 사용 예시
```bash
/create-pr                       # 현재 브랜치에서 자동으로 부모 브랜치 결정 (기본값: dev)
/create-pr dev                   # dev 브랜치를 base로 지정
/create-pr b2b/8.10.17           # b2b/8.10.17 브랜치를 base로 지정 (사용자 지정 우선)
...
if [[ -n "$ARGUMENTS" ]]; then
  ...
  if [[ -n "$PARENT" ]] && git branch -r | grep -q "origin/$PARENT"; then
    BASE_BRANCH="$PARENT"
  else
    BASE_BRANCH="dev"
  fi
