...
description: "GitHub 이슈 목록을 조회하여 사용자가 처리할 이슈를 선택하면 각 이슈에 대해 /b2c-ios-from-issue 스킬을 순차 호출하는 트리아지 스킬입니다. 라벨/작성자/상태로 필터링할 수 있고 복수 선택을 지원합니다."
argument-hint: "[옵션: --label=<name>] [--author=<login>] [--state=open|closed|all] [--limit=<n>] [dry-run]"
disable-model-invocation: false
allowed-tools: ["Bash", "Skill"]
---

# /b2c-ios-triage-issues - GitHub 이슈 트리아지

## 실행 알림
이 스킬이 실행되면 가장 먼저 "[b2c-ios-triage-issues] 스킬이 실행되었습니다."를 출력할 것

$ARGUMENTS

## 개요

여러 이슈를 한 번에 훑어보고 처리할 대상을 선택하는 **어댑터 스킬**이다. 실제 이슈 처리는 `/b2c-ios-from-issue` 스킬에 위임한다.

- 단일 이슈 처리: `/b2c-ios-from-issue <번호>` (직접 호출)
- 여러 이슈 트리아지: `/b2c-ios-triage-issues` (본 스킬)

## 처리 흐름

```
사용자: /b2c-ios-triage-issues [옵션]
   ↓
...
| `--limit=<n>` | `30` | 최대 조회 개수 |
| `dry-run` | off | Step 3 이후 실제 `/b2c-ios-from-issue` 호출 건너뜀 |
...
선택된 각 이슈에 대해 `/b2c-ios-from-issue` 스킬을 Skill 도구로 호출한다.

**호출 흐름:**
```
[1/3] #123 처리 시작
  → /b2c-ios-from-issue 123 호출
  → /b2c-ios-from-issue 내부: b2c-ios-issue-analyzer → 사용자 컨펌 → b2c-ios-notion-create → orchestrator
...
**중단 처리:**
- `/b2c-ios-from-issue` 내부에서 사용자가 취소(N)하면: 해당 이슈만 건너뛰고 다음으로 진행
- `/b2c-ios-from-issue` 실행 중 에러 발생: 에러 로그 수집 후 다음 이슈 처리 여부 사용자 확인

**dry-run 옵션:**
- `dry-run` 플래그가 있으면 이 단계에서 각 이슈에 대해 `/b2c-ios-from-issue <번호> dry-run`만 호출
...
```
/b2c-ios-triage-issues
```

### 버그 라벨만 보기
```
/b2c-ios-triage-issues --label=bug
```

### 특정 작성자의 이슈만
```
/b2c-ios-triage-issues --author=gu-gyodong
```

### 분석만 (실제 노션 생성/오케스트레이터 호출 없음)
```
/b2c-ios-triage-issues dry-run
```

### 복합 필터
```
/b2c-ios-triage-issues --label=bug --state=open --limit=10
```
...
| 개별 이슈 `/b2c-ios-from-issue` 실패 | 에러 기록 후 다음 이슈 진행 여부 확인 |
...
- **어댑터 스킬**이다: 실제 로직은 `/b2c-ios-from-issue`에 위임. 본 스킬은 목록 조회 + 선택 + 디스패치만 담당
- 대량 이슈(10개 이상) 트리아지는 시간이 오래 걸리므로, `dry-run`으로 먼저 훑어보길 권장
- 에픽급 이슈를 여러 개 묶어서 처리하려면 본 스킬 대신 수동으로 `/b2c-ios-from-issue`를 개별 호출하는 게 더 안전
- 이슈가 이미 PR에 연결된 경우(`Closes #N`) `/b2c-ios-from-issue` 내부에서 플래그되므로, 트리아지 단계에서 선택 시 주의
