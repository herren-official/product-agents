---
name: instaget-backend-pr-comment-reply
...
사용 시점:
- "PR 코멘트에 답글 달아줘", "reply to PR comment", "/instaget-backend-pr-comment-reply"
- PR 번호와 함께 사용 (예: "PR 1165 코멘트에 답글 달아줘")
- GitHub PR URL과 함께 사용 (예: "https://github.com/herren-official/instaget-server/pull/1168")
...
- **GitHub URL**: `https://github.com/herren-official/instaget-server/pull/1168`
- **URL + 코멘트 앵커**: `https://github.com/herren-official/instaget-server/pull/1168#discussion_r123456`
...
gh api repos/herren-official/instaget-server/pulls/comments/{COMMENT_ID} \
  --jq '{id, user: .user.login, path, line: (.line // .original_line), body}'

# 특정 일반 코멘트 조회 (issuecomment에서 추출한 ID)
gh api repos/herren-official/instaget-server/issues/comments/{COMMENT_ID} \
...
```bash
# 리뷰 코멘트 (인라인 코멘트) 조회
gh api repos/herren-official/instaget-server/pulls/{PR_NUMBER}/comments \
  --jq '.[] | {id, user: .user.login, path, line: (.line // .original_line), body: (.body | split("\n")[0:3] | join("\n")), in_reply_to_id, created_at}'

# 일반 이슈 코멘트 조회
gh api repos/herren-official/instaget-server/issues/{PR_NUMBER}/comments \
...
```bash
# PR 브랜치명 확인
gh pr view {PR_NUMBER} --repo herren-official/instaget-server --json headRefName --jq '.headRefName'
...
```bash
gh api repos/herren-official/instaget-server/pulls/{PR_NUMBER}/comments/{COMMENT_ID}/replies \
  -X POST \
  -f body='{reply_body}'
```
...
사용자: "https://github.com/herren-official/instaget-server/pull/1169#discussion_r2921399486 에 답글 달아줘"
