---
name: instaget-backend-refactoring-assistant
description: instaget-server 프로젝트 코드를 팀 컨벤션에 맞춰 분석하고, 멀티 모듈 영향도 분석을 통해 체계적으로 리팩토링합니다. "refactor", "apply conventions", "check code quality", "improve code", "find convention violations" 같은 요청에 사용.
---
...
Automated refactoring assistant for the instaget-server project that analyzes code against team conventions and applies systematic improvements with **multi-module impact analysis**.
...
2. **멀티 모듈 의존성 확인**
   ...
   infrastructure 모듈 변경 시 → common 제외 모든 모듈 확인
   - b2c_api ~ monitoring (위 목록에서 infrastructure 제외)
   ```
...
1. **컴파일 검증**
   ```bash
   # 영향 받는 모든 모듈 컴파일
   ./gradlew :common:compileKotlin :infrastructure:compileKotlin :back_office_api:compileKotlin :task_scheduler:compileKotlin

   # 테스트 코드 컴파일
   ./gradlew :common:compileTestKotlin :back_office_api:compileTestKotlin
   ```

2. **테스트 실행**
   ```bash
   # 영향 받는 테스트만 선택적 실행
   ./gradlew :common:test --tests "*ChangedClassTest"
   ./gradlew :task_scheduler:test --tests "*AffectedServiceTest"
   ```
...
- [ ] 전체 11개 모듈 확인 (infrastructure, b2c_api, back_office_api, task_scheduler, task_client, instagram_crawler_api, daily_task_crawler, instarter_order_automation, notification, monitoring)

**infrastructure 모듈 변경 시**:
- [ ] DB Entity 사용하는 모든 모듈 확인
- [ ] Repository 사용하는 모든 Service 확인
- [ ] Kafka DTO/토픽 변경 시 task_scheduler ↔ task_client 호환성 확인
- [ ] 크롤러 인터페이스 변경 시 instagram_crawler_api, daily_task_crawler 확인
